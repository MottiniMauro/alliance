...
M.init = function(conf)
	...
	for _, v in pairs( conf.sensors) do
		print('Initializing gray sensor '.. v[1] .. ' on Port ' .. v[2] .. ' triggering at ' .. v[3] ..' using '.. v[5] .. ' mode')	
		local device={}
		...
		local p = io.popen('find /sys/class/lego-sensor/sensor*')    
	   	for file in p:lines() do
			...
			if (sensor_type == 'lego-ev3-color' or sensor_type == 'lego-nxt-light') and
				...
				if sensor_type == 'lego-ev3-color' then mode_pref = 'COL-' end
			end
	   	end
		--local path_to_sensor = '/sys/class/lego-port/port'.. v[2]-1  ..'/ev3-ports\:in' .. v[2] ..'\:lego-'..v[3]..'-touch/lego-sensor/'
		--local sensor_folder = io.popen('ls '.. path_to_sensor):read()
		
		local mode_file_name = '/sys/class/lego-sensor/sensor'.. sensor_number ..'/mode'
		local mode_file = io.open(mode_file_name, 'w')
		mode_file:write(mode_pref..v[5])
		mode_file:close()
