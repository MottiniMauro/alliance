...
M.init = function(conf)
	...
	for _, v in pairs( conf.sensors) do
		print('Initializing distance sensor '.. v[1] .. ' on Port ' .. v[2] .. ' triggering at ' .. v[3])	
		local device={}
		
		local trigger_event={}
		device.name=v[1]
		device.module='distance'
		device.events={
		...
		local p = io.popen('find /sys/class/lego-sensor/sensor*')    
	   	for file in p:lines() do
			if io.open(file .. "/mode"):read() == 'US-DIST-CM' and 
		...
		local decimals = tonumber(io.open('/sys/class/lego-sensor/sensor'.. sensor_number ..'/decimals'):read())
		local val = 0
		device.task = sched.run(function()
			...
			while true do
				...
				local buff = tonumber(file:read())/(10.0 ^ decimals)
